...
*/
exports.uploadSaaSKPIFromCSV = async (req, res) => {
 ...
 try {
   ...
   const result = await saveKPIData(SaaSKPI, companyId, processedData, 'SaaS');

   return res.status(200).json({
     message: 'SaaS KPIs processed and uploaded successfully',
     data: result
   });

 } catch (error) {
   console.error('SaaS KPI upload error:', error);
...
exports.createSaaSKPI = async (req, res) => {
  try {
    const { companyId, companyName, selectedKPIs, kpis } = req.body;

    const newSaaSKPI = new SaaSKPI({ companyId, companyName, selectedKPIs, kpis });
    await newSaaSKPI.save();
    res.status(201).json({ message: 'SaaS KPI data added successfully', newSaaSKPI });
  } catch (error) {
...
exports.updateSaaSKPI = async (req, res) => {
  try {
    const { companyId } = req.params;
    const updatedData = req.body;

    const saasKpi = await SaaSKPI.findOneAndUpdate({ companyId }, updatedData, { new: true });
    if (!saasKpi) {
      return res.status(404).json({ message: 'Company not found' });
    }

    res.status(200).json({ message: 'SaaS KPI data updated successfully', saasKpi });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
exports.deleteSaaSKPI = async (req, res) => {
  try {
    const { companyId } = req.params;

    const saasKpi = await SaaSKPI.findOneAndDelete({ companyId });
    if (!saasKpi) {
      return res.status(404).json({ message: 'Company not found' });
    }

    res.status(200).json({ message: 'SaaS KPI data deleted successfully', saasKpi });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
exports.getSaaSKPI = async (req, res) => {
  try {
    const { companyId } = req.params;
    ...
    const chartData = formatChartData(saasKpi.data);

    res.status(200).json({
      selectedKPIs: saasKpi.selectedKPIs,
      kpis: chartData
    });
  } catch (error) {
    console.error('Error fetching SaaS KPI data:', error);
    res.status(500).json({ message: error.message });
  }
};
...
exports.getDashboardLayout = async (req, res) => {
  try {
    ...
    const result = await SaaSKPI.findOne(
      { companyId },
...
exports.saveDashboardLayout = async (req, res) => {
  try {
    ...
    const result = await SaaSKPI.findOneAndUpdate(
      { companyId },
...
exports.updateChartConfiguration = async (req, res) => {
  try {
    ...
    const document = await SaaSKPI.findOne({ companyId });

...
const LAYOUT_STORAGE_KEY = "manufacturing-dashboard-layouts";
const CHART_CONFIG_STORAGE_KEY = "manufacturing-dashboard-chart-config";
...
const ManufacturingDashboardPage = () => {
  ...
  const kpiMapping = {
    ...
    maintenanceCost: 'maintenanceCostPerUnit',
  ...
  const defaultLayout = [
    { i: "oee", x: 0, y: 0, w: 6, h: 4 },
    { i: "productionVolume", x: 6, y: 0, w: 6, h: 4 },
    { i: "cycleTime", x: 0, y: 4, w: 6, h: 4 },
    { i: "downtime", x: 6, y: 4, w: 6, h: 4 },
    { i: "yield", x: 0, y: 8, w: 6, h: 4 },
    { i: "scrapRate", x: 6, y: 8, w: 6, h: 4 },
    { i: "defectDensity", x: 0, y: 12, w: 6, h: 4 },
    { i: "maintenanceCost", x: 6, y: 12, w: 6, h: 4 },
    { i: "inventoryTurnover", x: 0, y: 16, w: 6, h: 4 },
    { i: "energyConsumption", x: 6, y: 16, w: 6, h: 4 }
  ];
  ...
  const defaultChartConfigurations = {
    ...
    yield: "LineChart",
    scrapRate: "AreaChart",
    defectDensity: "LineChart",
    maintenanceCost: "BarChart",
    inventoryTurnover: "BarChart",
    energyConsumption: "AreaChart"
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch KPI data
        const response = await axios.get(`${URL}/api/manufacturing/kpis/${COMPANY_ID}`);
        ...
        try {
          const layoutResponse = await axios.get(`${URL}/api/manufacturing/kpis/${COMPANY_ID}/layout`);
        ...
        try {
          const chartConfigResponse = await axios.get(`${URL}/api/manufacturing/kpis/${COMPANY_ID}/chartConfigurations`);
  ...
  const addKPI = async (kpiId) => {
    try {
      ...
      // Update on server
      await axios.put(`${URL}/api/manufacturing/kpis/${COMPANY_ID}/select`, { kpiId: kpiMapping[kpiId] });
  ...
  const deleteKPI = async (kpiId) => {
    try {
      ...
      // Update on server
      await axios.delete(`${URL}/api/manufacturing/kpis/${COMPANY_ID}/deselect/${kpiMapping[kpiId]}`);
  ...
  const onLayoutChange = (layout, allLayouts) => {
    ...
    if (!savingLayout) {
      ...
      setTimeout(async () => {
        try {
          await axios.put(`${URL}/api/manufacturing/kpis/${COMPANY_ID}/layout`, {
            layout: allLayouts
          });
  ...
  const handleChartTypeChange = async (kpiId, chartType) => {
    ...
    localStorage.setItem(CHART_CONFIG_STORAGE_KEY, JSON.stringify(updatedConfigs));
    ...
    if (!savingChartConfig) {
      ...
      setTimeout(async () => {
        try {
          await axios.put(`${URL}/api/manufacturing/kpis/${COMPANY_ID}/chartConfigurations/${kpiId}`, {
            chartType: chartType
          });
  ...
  const resetLayout = async () => {
    ...
    try {
      await axios.delete(`${URL}/api/manufacturing/kpis/${COMPANY_ID}/layout`);
    ...
    try {
      await axios.delete(`${URL}/api/manufacturing/kpis/${COMPANY_ID}/chartConfigurations`);
  ...
  const kpiConfigs = [
    ...
    },
    {
      ...
      chartType: chartConfigurations.yield || "LineChart",
      color: "#EC4899",
      xKey: "month",
      yKey: "value",
      icon: "✨"
    ...
    {
      id: "defectDensity",
      title: "Defect Density (per unit)",
      data: kpiData?.defectDensity || [],
      chartType: chartConfigurations.defectDensity || "LineChart",
      color: "#14B8A6",
      xKey: "month",
      yKey: "value",
      icon: "🔍"
    },
    {
      ...
      data: kpiData?.maintenanceCostPerUnit || [],
      chartType: chartConfigurations.maintenanceCost || "BarChart",
      color: "#06B6D4",
      xKey: "month",
      yKey: "value",
      icon: "💰"
    ...
    },
    {
      ...
      data: kpiData?.energyConsumptionPerUnit || [],
      chartType: chartConfigurations.energyConsumption || "AreaChart",
      color: "#0EA5E9",
      xKey: "month",
      yKey: "value",
      icon: "⚡"
  ...
  return (
    <div className="p-6 min-h-screen relative">
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold mb-4">Manufacturing Dashboard</h1>

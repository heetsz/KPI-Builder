...
const LAYOUT_STORAGE_KEY = "customer-dashboard-layouts";
const CHART_CONFIG_STORAGE_KEY = "customer-dashboard-chart-config";

// Add mapping between frontend KPI IDs and backend schema fields
const kpiMapping = {
  retention: 'customerRetentionRate',
  churn: 'customerChurnRate',
  ...
  onboarding: 'onboardingCompletionRate',
...
const CustomerDashboardPage = () => {
  ...
  // KPI visibility states - initialized as false, will be set based on selectedKPIs
  ...
  const defaultChartConfigurations = {
    ...
    cac: "AreaChart",
    onboarding: "LineChart",
    referral: "BarChart"
  };

  // Layout configuration
  const defaultLayout = [
    { i: "retention", x: 0, y: 0, w: 6, h: 4 },
    { i: "churn", x: 6, y: 0, w: 6, h: 4 },
    { i: "clv", x: 0, y: 4, w: 6, h: 4 },
    { i: "nps", x: 6, y: 4, w: 6, h: 4 },
    { i: "csat", x: 0, y: 8, w: 6, h: 4 },
    { i: "activeUsers", x: 6, y: 8, w: 6, h: 4 },
    { i: "conversion", x: 0, y: 12, w: 6, h: 4 },
    { i: "cac", x: 6, y: 12, w: 6, h: 4 },
    { i: "onboarding", x: 0, y: 16, w: 6, h: 4 },
    { i: "referral", x: 6, y: 16, w: 6, h: 4 },
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch KPI data
        const response = await axios.get(`${URL}/api/customer-growth/kpis/${COMPANY_ID}`);
        ...
        try {
          const layoutResponse = await axios.get(`${URL}/api/customer-growth/kpis/${COMPANY_ID}/layout`);
        ...
        try {
          const chartConfigResponse = await axios.get(`${URL}/api/customer-growth/kpis/${COMPANY_ID}/chartConfigurations`);
  ...
  const addKPI = async (kpiId) => {
    try {
      ...
      // Update on server - Map frontend kpiId to backend field name
      const backendKpiName = kpiMapping[kpiId];
      await axios.put(`${URL}/api/customer-growth/kpis/${COMPANY_ID}/select`, { kpiId: backendKpiName });
  ...
  const deleteKPI = async (kpiId) => {
    try {
      ...
      // Update on server
      await axios.delete(`${URL}/api/customer-growth/kpis/${COMPANY_ID}/deselect/${kpiId}`);
  ...
  const onLayoutChange = (layout, allLayouts) => {
    ...
    if (!savingLayout) {
      ...
      setTimeout(async () => {
        try {
          await axios.put(`${URL}/api/customer-growth/kpis/${COMPANY_ID}/layout`, {
            layout: allLayouts
          });
  ...
  const handleChartTypeChange = async (kpiId, chartType) => {
    ...
    if (!savingChartConfig) {
      ...
      setTimeout(async () => {
        try {
          await axios.put(`${URL}/api/customer-growth/kpis/${COMPANY_ID}/chartConfiguration/${kpiId}`, {
            chartType: chartType
          });
  ...
  const resetLayout = async () => {
    ...
    try {
      await axios.delete(`${URL}/api/customer-growth/kpis/${COMPANY_ID}/layout`);
    ...
    try {
      await axios.delete(`${URL}/api/customer-growth/kpis/${COMPANY_ID}/chartConfigurations`);
  ...
  const kpiConfigs = [
    {
      ...
      data: kpiData?.customerRetentionRate || [],
      chartType: chartConfigurations.retention || "LineChart",
      color: "#10B981",
      xKey: "month",
      yKey: "value",
      icon: "🔄"
    },
    {
      ...
      chartType: chartConfigurations.churn || "AreaChart",
      color: "#EF4444",
      xKey: "month",
      yKey: "value",
      icon: "👋"
    },
    {
      ...
      chartType: chartConfigurations.clv || "BarChart",
      color: "#8B5CF6",
      xKey: "month",
      yKey: "value",
      icon: "💰"
    },
    {
      ...
      data: kpiData?.netPromoterScore || [],
      chartType: chartConfigurations.nps || "RadarChart",
      color: "#F59E0B",
      xKey: "month",
      yKey: "value",
      icon: "⭐"
    },
    {
      ...
      data: kpiData?.customerSatisfactionScore || [],
      chartType: chartConfigurations.csat || "LineChart",
      color: "#06B6D4",
      xKey: "month",
      yKey: "value",
      icon: "😊"
    ...
    },
    {
      ...
      data: kpiData?.onboardingCompletionRate || [],
      chartType: chartConfigurations.onboarding || "LineChart",
      color: "#0EA5E9",
      xKey: "month",
      yKey: "value",
      icon: "✅"
    },
    {
      id: "referral",
      title: "Referral Rate (%)",
      data: kpiData?.referralRate || [],
      chartType: chartConfigurations.referral || "BarChart",
      color: "#A855F7",
      xKey: "month",
      yKey: "value",
      icon: "👥"
  ...
  return (
    <div className="p-6 min-h-screen relative">
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold mb-4">Customer Growth Dashboard</h1>
